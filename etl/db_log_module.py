...
def ins_suc_log(news_id, wrk_div, index_nm, st_dtm, end_dtm):
    try:
        con, cur = mysql_con()
...
def del_suc_log(news_id, wrk_div, index_nm, st_dtm, end_dtm):
    try:
        con, cur = mysql_con()
        prov_cd = news_id.split(".")[0]
        path = news_id.split(".")[1][:8]
        ...
        """
        sql_2 = """
            DELETE FROM ABKL_BIGKINDSLAB_NEWS WHERE NEWSITEMID = %s
        """

        cur.execute(sql_1, (news_id, prov_cd, path, wrk_div, index_nm, st_dtm, end_dtm))
        cur.execute(sql_2, (news_id))
        con.commit()
        con.close
...
def err_log(news_id, st_dtm, err_cd, err_cnts):
    try:
        con, cur = mysql_con()
        prov_cd = news_id.split(".")[0]
        path = news_id.split(".")[1][:8]
